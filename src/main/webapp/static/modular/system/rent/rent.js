...
Rent.initColumn = function () {
    return [
        ...
        {title: '已缴纳金额', field: 'price', align: 'center', valign: 'middle', sortable: true},
...
Rent.delete = function () {
    if (this.check()) {
        ...
        var operation = function(){
            ...
            ajax.set("rentId", Rent.seItem.id);
            ajax.start();
        };

        Feng.confirm("确认为用户缴费 " + Rent.seItem.name + "?", operation);
    }
};
...
$(function () {
    ...
    var table = new BSTable(Rent.id, "/rent/list", defaultColunms);
